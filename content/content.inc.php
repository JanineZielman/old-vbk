...
class content {
	 ...
	 */
	function __construct($siteFile, $hostname, $database, $username, $password, $tablePrefix = false) {
		// Save login data
	...
	function getAtom($id) {
		...
		if (is_numeric($id)) {
			...
			// Treat $id as slug
			$info = $this->_getDictionaryInfoBySlug($id);
		}
		if (!$info) {
			// Treat $id as id
	...
	function getAtoms($owner, $listInfo, $first = false, $number = false, $where = false, $order = false) {
		...
		// Get all atoms
		$query = "SELECT * FROM `$atomTable` WHERE (_owner='$owner') AND (_list='$listInfo->_name')";
		if ($where) {
			$query .= ' AND ('.$where.')';
		}
		$query .= ' '.$order;
	 ...
	 */
	function _getDictionaryInfoBySlug($slug) {
		// Have a look in the dictionary
		$query = "SELECT _id,_type,_slug FROM ".$this->tablePrefix."_dictionary WHERE _slug='$slug'";
		$this->db->query($query);
		if ($this->db->next_row()) {
			return $this->db->row;
		} else {
			...
			foreach ($fields as $fieldInfo) {
				if ($fieldInfo->_name==$fieldName) {
					if ($fieldInfo->_type==FIELD_TYPE_LINK) {
						...
						$replacements[] = htmlspecialchars($r, 0, 'iso-8859-1');
					}
				}
			}
		}
	...
	function _createSlug(&$atom) {
			...
			foreach ($fields as $fieldInfo) {
				foreach ($languages as $language) {
					...
					if ($extendedName==$fieldName) {
						...
						$replacements[] = htmlspecialchars(strip_tags($r), 0, 'iso-8859-1');
					}
				}
			}
		}
		$c1 = "äåâãàáçéèëêïîíìöòóôõü";
		$c2 = "aaaaaaceeeeiiiiooooou";
		...
		$info = $this->_getDictionaryInfoBySlug($slug);

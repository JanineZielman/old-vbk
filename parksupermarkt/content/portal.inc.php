...
class portal {
	...
	function getAtom($id, $type = false) {
		...
		return $this->content->_extendAtom($this->content->getAtom($id, $type), $lang);
	}
...
<?php
	...
	/**
	 * Returns the timestamp when the atom was last modified, or <code>false</code>
	 * if there is no atom with that ID or slug.
	 *
	 * @param mixed $id
	 * @return integer
	 */
	function getAtomTimestamp($id) {
		return $this->content->getAtomTimestamp($id);
	}

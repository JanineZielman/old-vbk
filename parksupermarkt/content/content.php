...
class cms {
	...
	function _getFieldDisplayValue($field, $atom) {
		...
		if ($field=='_label') {
			...
			$fieldValue = $atom->{$field} ? '<img class="listPicture" src="'.PATH_PREFIX.$atomField->_params['path'].$atom->{$field}.'?'.uniqid().'" />' : '';
...
<?php
		...
		foreach ($atoms as $atom) {
			...
			if (!$listInfo->_fixed) {
				...
				$commands .= '<a class="cmsButton" href="javascript:void(0)" onclick="deleteEntry(\''.$atom->_id.'\',\''.htmlspecialchars($this->_formatField($atom->_label)).'\');" title="'.$deleteTitle.'"><img src="elements/delete.gif" alt="'.$deleteTitle.'" title="'.$deleteTitle.'"></a>';
			}
			$row[] = $commands;
			// Print the row
					...
					case FIELD_TYPE_STRING:
						...
						foreach ($this->content->getLanguages() as $lang) {
							...
							$inputString = $this->page->input_text(
									$fieldNameLang,
									htmlspecialchars($atom->{$fieldNameLang} ? $atom->{$fieldNameLang} : $field->_params[FIELD_PARAM_DEFAULT]),
					...
					case FIELD_TYPE_CHARACTER:
						if ($field->_params[FIELD_PARAM_LINES]>1) {
							...
							$input = $this->page->input_memo(
									$field->_name,
									htmlspecialchars($atom->{$field->_name} ? $atom->{$field->_name} : $field->_params[FIELD_PARAM_DEFAULT]),
...
<?php
	...
	function checkServer() {
		...
		if (!is_integer(array_search('mysql', $extensions))) {
